...
interface Params {
    text: string,
    author: string,
    commuintyId: string | null,
    path: string,
}

export async function createThread({ text, author, commuintyId, path }: Params) {
    try {
        connectToDB();
...
export async function fetchPosts(pageNumber = 1, pageSize = 20) {
    ...
    try {
        ...
        // fetch all posts that have no parents (top-level threads...)
...
export async function fetchThreadById(id: string){
    ...
    try {
        ...
        throw new Error(`error of fetching thread ${error.message}`)
    }
}
...
export async function addCommentToThread(threadId: string, commentText: string, userId: string, path: string){
    ...
    try {
        ...
        if(!originalThread){
            throw new Error("There is no thread");
        }
